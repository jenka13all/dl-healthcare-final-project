...
def load_data(x_file, d_file, y_file):
    x_seq = np.array(pickle.load(open("./Med2Vec_data/" + x_file, 'rb')), dtype='object')
    d_seq = []
    if len(d_file) > 0:
        d_seq = np.array(pickle.load(open("./Med2Vec_data/" + d_file, 'rb')), dtype='object')
    y_seq = []
    if len(y_file) > 0:
        y_seq = np.array(pickle.load(open("./Med2Vec_data/" + y_file, 'rb')), dtype='object')
...
def model_train(med2vec, saver, config):
    for epoch in range(config['max_epoch']):
        ...
        if epoch == config['max_epoch'] - 1:
            saver.save(sess=med2vec.sess, save_path='./Med2Vec_model/200emb_200hidden/med2vec',
